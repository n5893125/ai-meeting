...
$source = Get-Location
$dest = "C:\Users\kyhsu\AiMeeting_Fix7"

Write-Host "Preparing FIXED source code (Round 7 - Dependency Fix) for GitHub upload..." -ForegroundColor Cyan
...
# --- REWRITE FILES WITH CORRECT SDK USAGE ---
...
# 3. functions/api/generate-image.ts
$genImageContent = @"
export async function onRequest(context: any) {
...
"@
Set-Content -Path "functions/api/generate-image.ts" -Value $genImageContent

# 4. functions/api/generate-report.ts
$genReportContent = @"
...
"@
Set-Content -Path "functions/package.json" -Value $funcPkgContent

Write-Host "Done! 'AiMeeting_Fix7' folder is ready." -ForegroundColor Green
Write-Host "Please upload content of C:\Users\kyhsu\AiMeeting_Fix7 to GitHub." -ForegroundColor Yellow
Invoke-Item .
